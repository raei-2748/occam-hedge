...
\subsection{Variance-Matched Control}
...
\midrule
Greeks-only (ERM)                           & \textbf{TBS} & \textbf{TBS} & \textbf{TBS} & -- \\
Micro-only (ERM)                            & \textbf{TBS} & \textbf{TBS} & \textbf{TBS} & -- \\
Combined (ERM)                              & \textbf{TBS} & \textbf{TBS} & \textbf{TBS} & -- \\
Combined ($L_2$, best $\lambda$)            & \textbf{TBS} & \textbf{TBS} & \textbf{TBS} & -- \\
Combined (VIB, best $\beta$)                & \textbf{TBS} & \textbf{TBS} & \textbf{TBS} & \textbf{TBS} \\
Oracle regime (Combined)                    & \textbf{TBS} & \textbf{TBS} & \textbf{TBS} & -- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\fbox{\parbox{0.9\linewidth}{\centering Placeholder: Bar chart of degradation ratio $R_1/R_0$ for Greeks-ERM, Micro-ERM, Combined-ERM, Combined-$L_2$, Combined-VIB.}}
\caption{Degradation under semantic inversion. ERM microstructure policies are expected to exhibit the largest $R_1/R_0$, while VIB reduces degradation by suppressing unstable dependence.}
\label{fig:deg_bar}
\end{figure}
...
\begin{figure}[t]
\centering
\fbox{\parbox{0.9\linewidth}{\centering Placeholder: Scatter plot with x-axis $R_0$, y-axis $R_1$, points for different $\beta$, colored by $\mathcal{C}(\phi)$.}}
\caption{Information--robustness trade-off (combined features). Each point is a VIB model at a different $\beta$. Color indicates information cost $\mathcal{C}(\phi)$ (darker = lower). Lower-information models are expected to exhibit smaller degradation (closer to $R_1=R_0$).}
\label{fig:frontier}
\end{figure}
...
This section links the empirical results back to the proposed information-economic mechanism and clarifies what would constitute strong evidence that the Regime~1 failure is primarily \emph{informational} (missing regime disambiguation) rather than merely \emph{structural} (insufficient model capacity or irrelevant features). We also benchmark Occam’s Hedge against standard regularization.
